...
public class FootstepMaster_Curves : MonoBehaviour {
	...
	// This Variable will get the Movement Speed from the Character Controller
	...
	void Update () {
		...
		// This Gets m_ForwardAmount from ThirdPersonCharacter Script
		move_Speed = gameObject.GetComponent<ThirdPersonCharacter>().m_ForwardAmount;
		//Debug.Log ("Speed: " + move_Speed);
	...
	//-----------------------------------------------------------------------------------------
	//Custom Part for Material Detection Instead of Texture
	/*void CheckMaterial(){
		floor.GetComponent<Renderer> ().materials [0].enableInstancing = true;
		floor.GetComponent<Renderer> ().materials [1].enableInstancing = true;
		//Debug.Log ("Material Name: " + floor.GetComponent<Renderer> ().materials);
		Debug.Log ("Material Name: " + floor.GetComponent<Renderer> ().materials);
		//floor.GetComponent<Renderer> ().materials.Length;
		if (floor.GetComponent<Renderer> ().materials[1].name == "grass (Instance)") {
			Invoke ("PlayGrass", 0);
			//Debug.Log ("It's Playing the grass Sound");
		} else 
			return;
		if (floor.GetComponent<Renderer> ().materials [0].name == "stone (Instance)") {
			Invoke ("PlayStone", 0);
		} else
			return;
		if (floor.GetComponent<Renderer> ().materials [2].name == "gravel (Instance)") {
			Invoke ("PlayGravel", 0);
		} else
			return;
	}*/
	...
	void CheckTexture(){
		...
		//Debug.Log("It's checking for a texture");
		if (floor.tag == ("Surface_Grass")) {
			Invoke ("PlayGrass", 0);
			Debug.Log ("It checked for grass tag!");
		}
		...
		if (floor.tag == ("Terrain")) {
			...
			if (whatTexture.Contains ("grass") == true || whatTexture.Contains ("Grass") == true || whatTexture.Contains ("GRASS") == true) {
				Invoke ("PlayGrass", 0);
				Debug.Log("It's detecting the Grass texture!");
			}
